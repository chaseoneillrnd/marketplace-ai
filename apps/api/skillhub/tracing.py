...
def setup_tracing(settings: Settings) -> trace.Tracer:
    ...
    When otel_traces_enabled is False, returns a NoOp tracer.
    When True, configures TracerProvider with OTLP exporter to Jaeger.
    Gracefully degrades if the exporter endpoint is unreachable.
    ...
    try:
        ...
        logger.warning("Failed to configure OpenTelemetry tracing, continuing without tracing", exc_info=True)
        _tracer = trace.get_tracer("skillhub")
